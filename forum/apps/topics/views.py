...
def index(request):
	latest_topics_list = Topic.objects.order_by('-id')[:3]
...
def create_post(request):
	topic = Topic(topic_title=request.POST['title'], topic_text=request.POST['text'], author_id = request.user.id)
	topic.save()
...
def detail(request, topic_id):
	...
	return render(request, 'topics/detail.html', {'topic': topic, 'latest_comments_list': latest_comments_list, 'name': name})
...
def delete_post(request, topic_id):
	try:
		topic = Topic.objects.get(id=topic_id)
		if request.user.is_authenticated and request.user.id == topic.author_id:
			topic.delete()
			return HttpResponseRedirect('/')
		else:
			raise Http404("Статья не найдена!")
...
def delete_comment(request, topic_id, c_id):
	try:
		...
		print(comment)
		if request.user.is_authenticated and request.user.id == comment.author_id:
